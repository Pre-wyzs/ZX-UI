...
<script>
	export default {
		...
		props: {
			...
			defaultchangevalue: {
				type: Array,
				default:() => [{label:'章子唯',value:'111'},{label:'王老牛',value:'222'}]
			},
			defaultrealValue: {
				type: Array,
				default:() => ['111','222']
		...
		created() {
			this.changevalue = this.defaultchangevalue
			this.realValue = this.defaultrealValue
		},
		methods: {
			close() {
				this.active = false
			...
			handleChange(index, item) {  
				...
				console.log('166',this.realValue)
				this.$emit('change', this.changevalue, this.realValue);
			}
		}
	};
